...
# `docs` branch on origin (Codeberg) — origin is the daemon repo, so the
# built docs sit next to the source on a sibling branch. Source for
# this project lives on `docs-src`.
#
...
echo "→ stage docs branch from _site/"
rm -rf .git
git init -q -b docs
git add .
